...
export function NotificationsTab({ dataSource = 'localStorage', adminReady = false }) {
    ...
    const load = async () => {
      try {
        if (dataSource === 'backend' && adminReady) {
          const res = await adminApiService.getNotifications();
          setAllNotifs((res.notifications || []).map(n => ({ ...n, userName: n.userName || n.userId })));
        } else {
          const all = profileService.getAllProfiles();
          setProfiles(all);
          const notifs = all.flatMap(p => (p.notifications || []).map(n => ({ ...n, userName: p.name || p.phoneNumber || p.email || p.userId })));
          notifs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
          setAllNotifs(notifs);
        }
      } catch (e) {
        console.warn('[NotificationsTab] backend load failed, falling back to localStorage:', e.message);
        const all = profileService.getAllProfiles();
        setProfiles(all);
        const notifs = all.flatMap(p => (p.notifications || []).map(n => ({ ...n, userName: p.name || p.phoneNumber || p.email || p.userId })));
        notifs.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  ...
  const handleSend = () => {
    ...
    profileService.addNotification(selectedUser, { type, message: message.trim() });
    setSent(true);
    setMessage('');
            ...
            <select className="admin-field-input" value={selectedUser} onChange={e => setSelectedUser(e.target.value)}>
              ...
              {profiles.map(p => (
                <option key={p.userId} value={p.userId}>
                  {p.businessBrain?.businessName || p.name || p.phoneNumber || p.email}
                </option>
              ))}
            </select>
          </div>
